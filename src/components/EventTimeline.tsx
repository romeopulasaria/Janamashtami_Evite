...
import { Sparkles, Heart, Bell, Star, Flame, Utensils, Feather, BookOpen, User } from "lucide-react";
import { InView } from "@/components/motion-primitives/in-view";
import { TextEffect } from "@/components/motion-primitives/text-effect";

// --- CSS Animations for background atmospheric floating notes & petals ---
const Atmospherics = () => (
...
export const EventTimeline: React.FC = () => {
  return (
    <div className="w-full relative overflow-hidden py-4 text-[#0a192f] font-inter">
      ...
      {/* Vertical Animated Spine (Visual Timeline Line Unfolding Downward) */}
      <div className="relative">
        <motion.div 
          ...
          className="absolute left-1/2 top-4 bottom-4 w-1 -translate-x-1/2 bg-gradient-to-b from-amber-400 via-amber-600 to-amber-400 opacity-30 origin-top rounded-full hidden md:block"
        />
              ...
              {[
                ...
                <InView key={idx} variant="blur-fade" delay={idx * 0.15}>
                  <motion.div 
                    ...
                    whileTap={{ scale: 0.97 }}
                    className="flex flex-col items-center text-center p-6 rounded-2xl bg-slate-50/50 hover:bg-white transition-all border border-transparent hover:border-amber-200/80 shadow-sm hover:shadow-md cursor-pointer"
                  >
